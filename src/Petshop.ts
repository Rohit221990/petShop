...
export const savedPets: (Cat | Dog)[] = [];

export function saveTest(): void {
  data.beginTran();
  let localSaved: (Cat | Dog)[] = [];
  try {
...
export function savePetShop(): void {
  data.beginTran();
  let localSaved: (Cat | Dog)[] = [];
...
export function logStats(): void {
  ...
  if (cats.length) {
    const avgCatNameLen =
      cats.reduce((acc, c) => acc + c.getAverageNameLength(), 0) / cats.length;
    console.log(`Average cat name length: ${avgCatNameLen.toFixed(2)}`);
  }
  if (dogs.length) {
    const avgDogNameLen =
      dogs.reduce((acc, d) => acc + d.getAverageNameLength(), 0) / dogs.length;
